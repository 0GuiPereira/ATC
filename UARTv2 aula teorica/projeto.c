...
#define BUFFERFULL 16
#define BUFFERSIZE 16 //FALTAVA
...
void configUART0(void) {
    ...
    REN0 = 1;
		//SCON0 |=0x10; //>> Ativa rececao timer0
	
    //Interrupcoes
    ES0 = 1; //Ativa a interr dporta serie (Reg IE.4)
...
void UARTInterrupt(void) interrupt 4 {
    if (RI0) {
        ...
        if (RxNumberOfData < BUFFERSIZE) {
            RxBuffer[RxBufWritePtr]=SBUF0 ;
            RxBufWritePtr = (RxBufWritePtr +1) % BUFFERSIZE;
            RxNumberOfData++;
        }
    }
    if (TI0) {
        ...
        TxNumberOfData--;
        if (TxNumberOfData) {
            SBUF0=TxBuffer[TxBufReadPtr];
            TxBufReadPtr = (TxBufReadPtr +1) % BUFFERSIZE;
            //TxNumberOfData--;
        }
    }
}
